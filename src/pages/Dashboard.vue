              ...
              >
                <template #append>
                  <q-icon name="extension" class="cursor-pointer">
                    <q-popup-proxy v-model="showLocationPicker">
                      <!-- TODO: Map or Geo search? -->
                    </q-popup-proxy>
                  </q-icon>
                </template>
              </q-input>
  ...
  computed: {
    ...
    containerStyle() {
      ...
      if (
        ...
        this.calendarView.endsWith("resource")
      ) {
        styles.height = "auto";
      } else {
  ...
  methods: {
            ...
            if (events.length > 0) {
              ...
              for (let j = 0; j < events.length; ++j) {
                ...
                if (evt.time !== undefined) {
                  const startTime2 = parseTimestamp(
                    evt.date + " " + evt.time
                  );
    ...
    checkDateTimeStart(/* val */) {
      ...
      if (this.eventForm.dateTimeStart && this.eventForm.dateTimeEnd) {
        ...
        const timestampEnd = parseTimestamp(
    ...
    addEventMenu({ scope, event }) {
      if (
        ...
        this.calendarView.indexOf("scheduler") > -1 ||
        this.calendarView.indexOf("agenda") > -1
      ) {
        return;
      }
      this.resetForm();
      ...
      this.$refs.event.validate().then(success => {
        if (success) {
          ...
          if (self.contextDay.bgcolor) {
            // TODO: Trigger an update to Calendar API
            console.log("Updated an event")
            update = true;
          } else {
            // TODO: Trigger an add to Calendar API
